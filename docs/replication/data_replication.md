...
NineData 支持 MySQL 到 MySQL、MySQL 到 ClickHouse、SQL Server 到 SQL Server 的数据复制功能。功能的具体支持情况请参见下表。

| 功能     | MySQL→MySQL | MySQL→ClickHouse | SQL Server→SQL Server |
| -------- | ----------- | ---------------- | --------------------- |
| 结构复制 | ✔️           | ✔️                | ✔️                     |
| 全量复制 | ✔️           | ✔️                | ✔️                     |
| 增量复制 | ✔️           | ✔️                | ❌                     |
...
3. 增量数据复制（SQL Server 暂不支持）：将源数据源中正在进行的数据变更实时同步到目标数据库。
...
* SQL Server 数据源在进行复制时，系统默认会排除 `master`、`model`、`msdb`、`tempdb`、`resource` 这 5 个系统库，因此这些库中的数据不会被复制。
* SQL Server 数据源当前仅支持结构复制和全量复制。
...
4. 在**数据源与目标**页签，按照下表进行配置，并单击**下一步**。
   ...
   | **源数据源**                           | 同步对象所在的数据源。                                       |
   | **目标数据源**                         | 接收同步对象的数据源。                                       |
   | **复制类型**                           | <ul><li>**结构复制**：只同步源数据源的库表结构，不同步数据。</li><li> **全量复制**：同步源数据源的所有对象和数据，即全量数据复制。</li><li>**增量复制**（SQL Server 暂不支持）：在全量同步完成后，基于源数据源的日志进行增量同步。</li></ul>**说明**：您还可以单击展开**高级设置**，选择存在同名表或相同数据时的处理策略。 |
...
7. 在**预检查**页签，等待系统完成预检查，预检查通过后，单击**启动任务**。
   ...
   - 您可以勾选**开启数据一致性对比**。在同步任务完成后，自动开启基于源数据源的数据一致性对比，保证两端数据一致。根据您选择的**复制类型**，数据一致性对比的启动时机如下：
     ...
     - **结构复制+全量复制+增量复制**（SQL Server 暂不支持）：当增量数据首次和源数据源一致且**同步延迟**为 0 秒时启动。您可以单击**查看详情**，在**复制详情**页面中查看同步延迟。![sync_delay](./image/sync_delay.png)
...
3. 在**数据复制**页面单击目标同步任务的**任务 ID**，页面说明如下。![result](./image/result1.png)
   ...
   | :------------------------------------: | -------------------------------------- | ------------------------------------------------------------ |
   |                   1                    | 同步延迟（SQL Server 暂不支持）        | 源数据源和目标数据源之间的增量数据同步延迟，0 秒表示两端之间没有延迟，此时您可以选择将业务切换到目标数据源，实现平滑迁移。 |
