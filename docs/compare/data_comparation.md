...
玖章云数据库对比功能支持数据对比，即用户数据（UserData）的对比，用于对比两个数据库中的数据是否一致。通常用于备份恢复、数据迁移、数据同步后两端的数据一致性对比。本文介绍如何进行数据对比。
...
2. 在左侧导航栏单击**数据库对比**>**数据对比**。![data_compare](./image/data_compare.png)

3. 在**数据对比**页面的右上角，单击**创建数据对比**。![create_dctask](./image/create_dctask.png)

4. 在**选择数据源**页签，按照下表进行配置，并单击**下一步**。![config1](./image/dc_config1.png)
...
5. 在**选择对比对象**页签，选择需要对比的内容，并单击**下一步**。支持**全实例**和**部分对象**。![dc_config2](./image/dc_config2.png)

   - **全实例**：选择源数据源中所有的数据库及对象。
   - **部分对象**：自定义选择需要进行对比的数据库及对象。在左侧**源对象**列表中，勾选需要对比的数据库及对象，单击**>**添加到**目标对象**列表。

6. 在**配置映射关系**页签，配置源端和目标端的数据库及对象名称的映射关系，并单击**保存并预检查**。![dc_config3](./image/dc_config3.png)
...
7. 在**预检查**页签，等待系统完成预检查，预检查通过后，单击**启动任务**。![dc_config4](./image/dc_config4.png)

   > * 如果预检查未通过，需要单击目标检查项右侧**操作**列的**详情**，排查失败的原因，手动修复后重新执行预检查，直到通过。
   > * **检查结果**为**警告**的检查项，可视具体情况修复或忽略。

8. 在**启动任务**页面，提示**启动成功**。![dc_config5](./image/dc_config5.png)您可以执行如下操作：
