...
玖章云支持对将数据源中的某一列或多个列设置为敏感列，未被授权查看敏感列的用户将无法查看该列的内容。

### 前提条件

已将需要添加敏感字段的数据源添加到玖章云。如何添加，请参见[添加数据源](datasource.md)。
...
4. 单击**数据源**下方的选框，会弹出已添加到玖章云的数据源列表，从列表中选择需要添加敏感列的数据源。

   >  如果您没有找到目标数据源，请单击列表下方的**立即创建**，添加数据源。更多信息，请参见[添加数据源](datasource.md)。

5. 玖章云支持**手动添加**和**扫描添加**两个选项，其中**扫描添加**可以根据[识别规则](#rules)自动扫描数据源中的敏感列并添加到敏感列列表。
...
3. 单击**敏感列**页签，这里会列出当前玖章云账号下所有数据源中的敏感列，您可以做如下操作：
...
识别规则用于系统自动扫描数据源中的敏感列，玖章云提供了10条默认的[识别规则](#rules)，这些规则无法被修改。您也可以根据如下步骤创建自定义识别规则。
...
脱敏算法是对敏感列进行脱敏的方式，玖章云提供了13条默认的[脱敏算法](#algorism)，这些算法无法被修改。您也可以根据如下步骤创建自定义脱敏算法。
